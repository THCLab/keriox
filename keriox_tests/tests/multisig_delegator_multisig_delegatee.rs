use keri_controller::{error::ControllerError, mailbox_updating::ActionRequired, KeyManager, SelfSigningPrefix};
...
#[async_std::test]
async fn multi_delegator_multi_delegatee(ctx: &mut InfrastructureContext) -> Result<(), ControllerError> {
    ...
    let (mut delegator_identifier1, delegator_km1, controller1) = setup_identifier(root_0.path(), vec![witness_oobi.clone()], None, None).await;
    ...
    let (mut delegator_identifier2, delegator_km2, controller2) = setup_identifier(root_1.path(), vec![witness_oobi.clone()], None, None).await;
	...
	for msg in id2_kel {
		delegator_identifier1.known_events.process(&Message::Notice(msg)).unwrap();
	}

    let state = delegator_identifier1.find_state(delegator_identifier2.id()).unwrap();
	...
	for msg in id1_kel {
		delegator_identifier2.known_events.process(&Message::Notice(msg)).unwrap();
	}

    let state = delegator_identifier2.find_state(delegator_identifier1.id()).unwrap();
    assert_eq!(state.sn, 0);
    ...
    let signature_icp = SelfSigningPrefix::Ed25519Sha512(delegator_km1.sign(group_inception.as_bytes())?);
    let signature_exn = SelfSigningPrefix::Ed25519Sha512(delegator_km1.sign(exn_messages[0].as_bytes())?);
    ...
    let query = delegator_identifier2.query_mailbox(&delegator_identifier2.id(), &[witness_id.clone()])?;
    ...
    for qry in query {
        ...
        match &action_required[0] {
            ...
            ActionRequired::MultisigRequest(multisig_event, exn) => {
                let signature_ixn =
                    SelfSigningPrefix::Ed25519Sha512(delegator_km2.sign(&multisig_event.encode()?)?);
                let signature_exn = SelfSigningPrefix::Ed25519Sha512(delegator_km2.sign(&exn.encode()?)?);
    ...
    println!("\tparticipants: {:?}", &[&delegator_identifier1, &delegator_identifier2].iter().map(|part| part.id().to_string()).collect::<Vec<_>>());


    // Setup delegatee identifier. It will be multisig group.
    // Setup identifier for first group participant.
    let delegatee_root_0 = Builder::new().prefix("test-db01").tempdir().unwrap();
    let (mut delegatee_identifier1, delegatee_km1, delegatee_controller1) = setup_identifier(delegatee_root_0.path(), vec![witness_oobi.clone()], None, None).await;
   
    assert!(delegatee_identifier1.get_own_kel().is_some());

    // Setup identifier for second group participant.
    let delegatee_root_1 = Builder::new().prefix("test-db02").tempdir().unwrap();
    let (mut delegatee_identifier2, delegatee_km2, delegatee_controller2) = setup_identifier(delegatee_root_1.path(), vec![witness_oobi.clone()], None, None).await;
	...
	for msg in id2_kel {
		delegatee_identifier1.known_events.process(&Message::Notice(msg)).unwrap();
	}

    let state = delegatee_identifier1.find_state(delegatee_identifier2.id()).unwrap();
    assert_eq!(state.sn, 0);

    // Provide first identifier's KEL to second identifier.
	let id1_kel = delegatee_identifier1.get_own_kel().unwrap();
	for msg in id1_kel {
		delegatee_identifier2.known_events.process(&Message::Notice(msg)).unwrap();
	}

    let state = delegatee_identifier2.find_state(delegatee_identifier1.id()).unwrap();
    assert_eq!(state.sn, 0);
    ...
    let signature_icp = SelfSigningPrefix::Ed25519Sha512(delegatee_km1.sign(delegatee_group_inception.as_bytes())?);
    let exns = exn_messages.into_iter().map(|exn| {
        let signature_exn = SelfSigningPrefix::Ed25519Sha512(delegatee_km1.sign(exn.as_bytes()).unwrap());
    ...
    let delegatee_group_id = delegatee_identifier1
        .finalize_group_incept(
            delegatee_group_inception.as_bytes(),
            signature_icp,
            exns,
        )
        .await?;
    ...
    // Querying mailbox to get multisig request
    let query = delegatee_identifier2.query_mailbox(&delegatee_identifier2.id(), &[witness_id.clone()])?;
    ...
    for qry in query {
        ...
        match &action_required[0] {
            ...
            ActionRequired::MultisigRequest(multisig_event, exn) => {
                let signature_ixn =
                    SelfSigningPrefix::Ed25519Sha512(delegatee_km2.sign(&multisig_event.encode()?)?);
                let signature_exn = SelfSigningPrefix::Ed25519Sha512(delegatee_km2.sign(&exn.encode()?)?);
    ...
    handle_delegation_request(&mut delegator_identifier1, &delegator_km1, &[witness_id.clone()], delegator_group_id.clone(), &delegatee_group_id).await.unwrap();
    handle_delegation_request(&mut delegator_identifier2, &delegator_km2, &[witness_id.clone()], delegator_group_id.clone(), &delegatee_group_id).await.unwrap();

    // ixn was accepted. 
    let delegator_state = controller2.find_state(&delegator_group_id).unwrap();
    let kel = controller2.get_kel_with_receipts(&delegator_group_id).unwrap();
    assert_eq!(delegator_state.sn, 1);

    println!("\nDelegator group identifier: {}", &delegator_group_id);
    println!("First group identifier: {}", &delegator_identifier1.id());
    println!("Second group identifier: {}", &delegator_identifier2.id());
    println!("Delegatee group identifier: {}", &delegatee_group_id);
    println!("First group identifier: {}", &delegatee_identifier1.id());
    println!("Second group identifier: {}", &delegatee_identifier2.id());
    ...
    for qry in query {
        let signature =
            SelfSigningPrefix::Ed25519Sha512(delegator_km1.sign(&qry.encode()?)?);
    ...
    let delegators_kel = controller1
        ...
        .unwrap();
    delegatee_identifier1
        .known_events.process(&Message::Notice(delegators_kel[0].clone()))?; // icp
    ...
    let state = delegatee_identifier1.find_state(&delegatee_group_id).unwrap();
    assert_eq!(state.sn, 0);

    let state = delegatee_identifier1.find_state(&delegator_group_id).unwrap();
    ...
    let state = delegatee_identifier2.find_state(&delegatee_group_id).unwrap();
    assert_eq!(state.sn, 0);

    let state = delegatee_identifier2.find_state(&delegator_group_id).unwrap();
    assert_eq!(state.sn, 1);
