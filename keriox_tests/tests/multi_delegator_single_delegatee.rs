...
#[async_std::test]
async fn multi_delegator_single_delegatee(ctx: &mut InfrastructureContext) -> Result<(), ControllerError> {
    ...
    let (mut identifier1, km1, controller1) = setup_identifier(root_0.path(), vec![witness_oobi.clone()], None, None).await;
    ...
    let (mut identifier2, km2, controller2) = setup_identifier(root_1.path(), vec![witness_oobi.clone()], None, None).await;
	...
	for msg in id2_kel {
		identifier1.known_events.process(&Message::Notice(msg)).unwrap();
	}
	...
	for msg in id1_kel {
		identifier2.known_events.process(&Message::Notice(msg)).unwrap();
	}
    ...
    println!("\tparticipants: {:?}", &[&identifier1, &identifier2].iter().map(|part| part.id().to_string()).collect::<Vec<_>>());
    ...
    let (mut temporary_delegatee_identifier, delegatee_keypair, _) = setup_identifier(delegatee_root.path(), vec![witness_oobi], None, None).await;
    ...
    handle_delegation_request(&mut identifier1, &km1, &[witness_id.clone()], delegator_group_id.clone(), &delegatee_id).await.unwrap();
    handle_delegation_request(&mut identifier2, &km2, &[witness_id.clone()], delegator_group_id.clone(), &delegatee_id).await.unwrap();
    ...
    println!("Delegator group identifier: {}", &delegator_group_id);
    println!("First group identifier: {}", &identifier1.id());
    println!("Second group identifier: {}", &identifier2.id());
    let query = identifier1.query_mailbox(&delegator_group_id, &[witness_id.clone()])?;

    for qry in query {
        let signature =
            SelfSigningPrefix::Ed25519Sha512(km1.sign(&qry.encode()?)?);
    ...
    let query = temporary_delegatee_identifier.query_mailbox(&delegatee_id, &[witness_id.clone()])?;
