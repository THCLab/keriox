...
//     let mut oobis_db_path =  Builder::new().prefix("test-oobi").tempdir().unwrap();
...
//     // Init alice.
//     let mut alice = Controller::new(Arc::clone(&db_alice), alice_key_manager.clone(), oobis_db_path.path())?;
...
//     // Init bob.
//     let mut bob = Controller::new(Arc::clone(&db_bob), bob_key_manager.clone(), oobis_db_path.path())?;
