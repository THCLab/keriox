...
#[cfg(test)]
mod test;
use keri_core::{actor::{event_generator, prelude::SelfAddressingIdentifier}, event::{event_data::EventData, KeyEvent}, event_message::{cesr_adapter::{parse_event_type, EventType}, msg::KeriEvent, signed_event_message::{Message, Notice, SignedEventMessage}}, oobi::{LocationScheme, Scheme}, prefix::{BasicPrefix, IdentifierPrefix, IndexedSignature, SelfSigningPrefix}, query::reply_event::{ReplyEvent, ReplyRoute}};
...
impl Identifier {
	pub fn new(id: IdentifierPrefix, known_events: Arc<KnownEvents>, communication: Arc<Communication>) -> Self {
		Self { id, known_events, communication, to_notify: vec![] }
	}
    ...
    fn finalize_key_event(
        ...
        self.known_events.save(&Message::Notice(Notice::Event(signed_message.clone())))?;
	...
	async fn finalize_add_role(
        ...
        sig: Vec<SelfSigningPrefix>,
			...
			let (dest_identifier, messages_to_send) = self.known_events.finalize_add_role(signer_prefix, event, sig)?;
			// TODO: send in one request
			for ev in messages_to_send {
				self.communication.send_message_to(&dest_identifier, Scheme::Http, ev)
					.await?;
			}
    ...
    pub(crate) fn get_index(&self, key_event: &KeyEvent) -> Result<usize, ControllerError> {
        match &key_event.event_data {
            ...
            EventData::Ixn(_ixn) => {
                let own_pk = self.known_events.current_public_keys(&self.id)?[0].clone();
                self.known_events.current_public_keys(&key_event.get_prefix())?
                    .iter()
                    .position(|pk| pk.eq(&own_pk))
            }
        }
