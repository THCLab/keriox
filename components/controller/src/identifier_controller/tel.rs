...
impl IdentifierController {
    ...
    /// the process, `ixn` need to be signed confirmed with `finalize_event`
    /// function.
    pub fn issue(&self, credential_sai: SelfAddressingIdentifier) -> Result<(IdentifierPrefix, Vec<u8>), ControllerError> {
        match self.registry_id.as_ref() {
            Some(registry_id) => {
                let tel = self.source.tel.clone();
                let iss =
                    tel.make_issuance_event(registry_id, credential_sai)?;
