...
fn interop() -> Result<(), Error> {
    ...
    let (processor, storage) = {
        ...
        (
            EventProcessor::new(db.clone()),
            EventStorage::new(db.clone()),
        )
    };
    parsed_issuer_kel.into_iter().for_each(|msg| {
        let not = processor.process(msg).unwrap();
        println!("{:?}", not);
    });
    // parsed_holder_kel.into_iter().for_each(|msg| {processor.process(msg).unwrap();});
    // let pk = storage.get_state(&"Ew-o5dU5WjDrxDBK4b4HrF82_rYb6MX6xsegjq4n0Y7M".parse().unwrap()).unwrap().unwrap();
    // let pk = storage.get_state(&"EeWTHzoGK_dNn71CmJh-4iILvqHGXcqEoKGF4VUc6ZXI".parse().unwrap()).unwrap().unwrap();
