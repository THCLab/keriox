...
async fn main() -> std::io::Result<()> {
    ...
    let watcher_prefixes = vec![
        "Bo1r-yOiKo7tZw_NEDM6fHeH4ljnp5FJqb3NNG4dwbig",
        ]
    .iter()
    .map(|prefix_str| prefix_str.parse::<IdentifierPrefix>().unwrap())
    .collect::<Vec<_>>();

    let watcher_addresses = vec![
        "http://localhost:3236",
    ];
    ...
    .await;
    let issuer_id = "EMI1a7MCfDG_BwE4kGTt5K7fmF3pbKHKy4xE-Ajb1u_Y".parse::<IdentifierPrefix>().unwrap();
