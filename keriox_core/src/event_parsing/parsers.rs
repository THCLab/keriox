...
use nom::{bytes::complete::take, error::ErrorKind, sequence::tuple, multi::count};

use super::{codes::{self_signing::SelfSigning, basic::Basic, self_addressing::SelfAddressing, serial_number::SerialNumberCode, parse_value}};

use super::codes::{group::GroupCode, IndexedSignature, attached_signature_code::AttachedSignatureCode, Signature, NontransferableIdentifier, Digest, Identifier, IdentifierCode, TransferableQuadruple, IdentifierSignaturesCouple};
...
fn test_basic_identifier() {
	let pk_raw = vec![249, 247, 209, 34, 220, 90, 114, 42, 247, 149, 69, 221, 219, 244, 123, 60, 41, 37, 217, 217, 199, 132, 199, 134, 143, 65, 11, 79, 135, 11, 85, 16];
...
#[test]
fn test_digest() {
	let digest_raw = vec![176, 185, 47, 120, 129, 84, 62, 251, 119, 243, 24, 109, 129, 134, 9, 68, 32, 169, 0, 99, 187, 90, 56, 199, 85, 29, 251, 61, 172, 47, 235, 177];
...
fn test_signature() {
	...
	let signature_raw = vec![181, 80, 26, 245, 64, 62, 83, 162, 71, 92, 156, 239, 197, 154, 122, 24, 177, 223, 43, 7, 143, 254, 161, 164, 45, 147, 71, 95, 21, 36, 151, 61, 204, 64, 187, 186, 82, 105, 2, 21, 126, 72, 115, 114, 64, 149, 153, 50, 32, 173, 220, 114, 93, 45, 181, 185, 152, 44, 236, 220, 104, 201, 144, 16];

    assert_eq!(
        signature(string_to_parse.as_bytes()),
        Ok(("more".as_bytes(), (SelfSigning::Ed25519Sha512, signature_raw)))
    );
}
