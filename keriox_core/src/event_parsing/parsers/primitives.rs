...
pub fn parse_primitive<C: DerivationCode + FromStr<Err = Error>>(
    ...
    let code = C::from_str(std::str::from_utf8(stream).unwrap())
        .map_err(|_| nom::Err::Error((stream, ErrorKind::IsNot)))?;
    ...
    let decoded = from_text_to_bytes(data)
        .map_err(|_| nom::Err::Error((stream, ErrorKind::IsNot)))?[code.code_size() % 4..]
        .to_vec();
...
pub fn serial_number_parser(s: &[u8]) -> nom::IResult<&[u8], u64> {
    ...
    let (rest, data) = take(code.value_size() as usize)(rest)?;
    let decoded = from_text_to_bytes(data).map_err(|_| nom::Err::Error((s, ErrorKind::IsNot)))?
        [code.code_size()..]
        .to_vec();
...
pub fn material_path(s: &[u8]) -> nom::IResult<&[u8], MaterialPath> {
    ...
    let payload_type: MaterialPathCode = std::str::from_utf8(type_c)
        .unwrap()
        .parse()
