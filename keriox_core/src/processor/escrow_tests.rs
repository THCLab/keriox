...
fn test_process_transferable_receipt() -> Result<(), Error> {
    ...
    let parsed = parse::<EventType>(icp_raw).unwrap().1;
    ...
    let parsed = parse::<EventType>(vrc_raw).unwrap().1;
    ...
    let parsed = parse::<EventType>(val_icp_raw).unwrap().1;
...
pub fn test_not_fully_witnessed() -> Result<(), Error> {
    ...
    let parsed_icp = parse::<EventType>(icp_raw).unwrap().1;
    ...
    let parsed_rcp = parse::<EventType>(receipt0_0).unwrap().1;
    ...
    let parsed_rcp = parse::<EventType>(receipt0_1).unwrap().1;
    let rcp_msg = Message::try_from(parsed_rcp).unwrap();
    event_processor.process(&rcp_msg.clone())?;
    ...
    let parsed_rcp = parse::<EventType>(receipt0_2).unwrap().1;
    let rcp_msg = Message::try_from(parsed_rcp).unwrap();
    event_processor.process(&rcp_msg.clone())?;
...
pub fn test_reply_escrow() -> Result<(), Error> {
    ...
    let parsed = parse_many::<EventType>(kel.as_bytes()).unwrap().1;
    ...
    let parsed = parse_many::<EventType>(rest_of_kel.as_bytes()).unwrap().1;
    ...
    let parsed = parse::<EventType>(old_rpy.as_bytes()).unwrap().1;
    ...
    let parsed = parse::<EventType>(new_rpy.as_bytes()).unwrap().1;
...
fn test_partially_sign_escrow() -> Result<(), Error> {
    ...
    let parse_messagee = |raw_event| {
        let parsed = parse::<EventType>(raw_event).unwrap().1;
